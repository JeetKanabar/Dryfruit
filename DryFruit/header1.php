...
.secnav .user-container button:hover {
  background-color: #6fdeff;
}

.thenav .search-container {
	height:55px;
  background-color:#e6f0ff;
  padding: 6px 40px;
  overflow:hidden;  
...
.thenav .search-container button {
  background: #6faaff;
  font-size: 20px;
  border: none;
  cursor: pointer;
...
.thenav .search-container button:hover {
  background: #ccc;
}
...
<div class="topnav">
  ...
  <a href="orderdisp.php">YOUR CART</a>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    searchIconContainer.addEventListener("click", function() {
      if (searchContainer.style.display === "none" || userContainer.style.display === "") {
        ...
        searchContainer.style.display = "none";
        searchIconContainer.innerHTML = '<i class="fa fa-user" aria-hidden="true"></i> USER';
      }
    });
  });
  </script>
